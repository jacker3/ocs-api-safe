...
# ============ CIRCUIT BREAKER ============
def is_circuit_open():
    """Проверка, открыт ли предохранитель"""
    if circuit_breaker['open_until']:
        if datetime.now().timestamp() < circuit_breaker['open_until']:
            return True
        else:
...
def record_failure():
    """Запись ошибки в предохранитель"""
    circuit_breaker['failures'] += 1
    circuit_breaker['last_failure'] = datetime.now()
    if circuit_breaker['failures'] >= circuit_breaker['threshold']:
        circuit_breaker['open_until'] = datetime.now().timestamp() + circuit_breaker['timeout']
        logger.warning(f"🔌 Circuit breaker OPEN for {circuit_breaker['timeout']}s")

def record_success():
    """Запись успеха в предохранитель"""
...
def get_timeout_for_endpoint(endpoint, category=None):
    ...
    if '/categories' in endpoint and '/products' not in endpoint:
        if '/page/' in endpoint:
            return TIMEOUTS['categories_page']
        return TIMEOUTS['categories']
    elif '/products' in endpoint:
        if '/page/' in endpoint:
            return TIMEOUTS['products_page']
...
class OCSClient:
    ...
    def _make_request_with_retry(self, method, endpoint, params=None, data=None,
        ...
        if is_circuit_open():
            return {
                'error': 'Circuit breaker open - OCS API unavailable',
                'retry_after': circuit_breaker['open_until'] - datetime.now().timestamp()
            }, 0, False, 503
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                # ✅ ИСПРАВЛЕНИЕ: Разделяем GET и POST запросы
                if method.upper() == 'GET':
                    response = self.session.request(
                        method=method,
                        url=url,
                        params=params,
                        # ❌ УБРАНО: json=data для GET запросов
                        timeout=timeout,
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"✗ Method Not Allowed (405) for {endpoint}")
                    record_failure()
                    return {'error': f'Method {method} not allowed, use GET'}, elapsed, False, 405
                    ...
                    record_failure()
                    return {'error': f'Authentication failed ({response.status_code})'}, elapsed, False, response.status_code
                
                elif response.status_code == 503:
                    logger.warning(f"⚠️ Service Unavailable (503) for {endpoint}")
                    record_failure()
                    if attempt < max_retries:
                        time.sleep(2 ** attempt)
                        continue
                    return {'error': 'OCS API temporarily unavailable (503)'}, elapsed, False, 503
...
def health():
    ...
    checks['circuit_breaker'] = 'open' if is_circuit_open() else 'closed'
